...
function openProjectDialog(id=""){
  ...
  form.elements.id.value=project?.id||"";form.elements.name.value=project?.name||"";form.elements.code.value=project?.code||"";form.elements.status.value=project?.status||"Active";form.elements.budget.value=project?.budget?String(project.budget):"";form.elements.expectedEndDate.value=project?.expectedEndDate||"";form.elements.endDate.value=project?.endDate||"";form.elements.notes.value=project?.notes||"";
...
async function submitProject(e){
  e.preventDefault();const form=e.currentTarget,fd=new FormData(form);const project={id:String(fd.get("id")||"")||undefined,name:String(fd.get("name")||"").trim(),code:String(fd.get("code")||"").trim(),status:String(fd.get("status")||"Active"),budget:Number(fd.get("budget"))||0,expectedEndDate:String(fd.get("expectedEndDate")||""),endDate:String(fd.get("endDate")||""),notes:String(fd.get("notes")||"").trim(),projectParts:collectProjectDirectParts(),manualParts:[]};
  try{
