...
function ensureProjectsUi(){
  ...
  [$("#projectOrderedPartsEditor"),$("#projectUsedPartsEditor")].forEach(host=>{if(!host)return;host.addEventListener("click",event=>{const remove=event.target.closest("[data-project-direct-remove]");if(remove)remove.closest(".project-direct-part")?.remove();});host.addEventListener("change",event=>{const select=event.target.closest("[data-project-catalog]");if(!select)return;const row=select.closest(".project-direct-part");const part=partCatalog.find(item=>String(item.id)===String(select.value));if(!row)return;if(part){row.dataset.catalogPartId=String(part.id);const name=row.querySelector("[data-project-name]"),code=row.querySelector("[data-project-code]"),price=row.querySelector("[data-project-price]");if(name)name.value=part.name||"";if(code)code.value=part.partNo||"";if(price&&!String(price.value||"").trim()){const remembered=lastPurchasePrice(part.name,part.partNo,"");if(remembered)price.value=Number(remembered.price).toFixed(2);}}else{row.dataset.catalogPartId="";}});});
...
function addProjectDirectPartRow(kind,data={}){
  ...
  const supplierNames=activeSuppliers();
  const supplierChecksHtml=supplierNames.length?supplierNames.map(name=>`<label class="project-supplier-option"><input type="checkbox" data-project-supplier value="${esc(name)}" ${selectedSuppliers.has(String(name))?'checked':''}><span>${esc(name)}</span></label>`).join(""):`<div class="project-supplier-empty">No suppliers saved yet.</div>`;
  const supplierSummary=selectedSuppliers.size?[...selectedSuppliers].join(", "):"Select suppliers…";
  ...
  row.innerHTML=`<div class="part-entry-head"><strong>${kind==="ordered"?"Ordered part":"Used part"} ${projectDirectPartCounter}</strong><button type="button" class="remove-part-btn" data-project-direct-remove>Remove</button></div><div class="part-entry-grid project-part-entry-grid"><label>Part<select data-project-catalog>${projectCatalogOptions(data.catalogPartId||"")}</select></label><label>Part name<input data-project-name maxlength="180" value="${esc(data.name||"")}" placeholder="e.g. Anvil"></label><label>Part code<input data-project-code maxlength="100" value="${esc(data.partNo||"")}" placeholder="Optional"></label><label>${kind==="ordered"?"Quantity ordered":"Quantity used"}<input data-project-qty type="number" min="0" step="0.01" value="${qty||1}"></label><label>Unit cost (${currencySymbol()})<input data-project-price type="number" min="0" step="0.01" value="${data.unitPrice!==undefined?Math.max(0,Number(data.unitPrice)||0):""}" placeholder="0.00"></label><label>Suppliers<details class="project-supplier-picker"><summary data-project-supplier-summary>${esc(supplierSummary)}</summary><div class="project-supplier-menu">${supplierChecksHtml}</div></details><span class="project-manual-help">Select one or more suppliers</span></label></div><label class="project-manual-stock"><input data-project-add-parts type="checkbox" ${data.addToInventory||alreadyInParts?'checked':''}> ${alreadyInParts?'In Parts inventory':'Add to Parts inventory'}</label>`;
  host.appendChild(row);
  row.addEventListener("change",event=>{
    if(!event.target.matches("[data-project-supplier]"))return;
    const selected=[...row.querySelectorAll("[data-project-supplier]:checked")].map(input=>String(input.value||"").trim()).filter(Boolean);
    const summary=row.querySelector("[data-project-supplier-summary]");
    if(summary)summary.textContent=selected.length?selected.join(", "):"Select suppliers…";
  });
}
...
async function submitProject(e){
  e.preventDefault();const form=e.currentTarget,fd=new FormData(form);const project={id:String(fd.get("id")||"")||undefined,name:String(fd.get("name")||"").trim(),code:String(fd.get("code")||"").trim(),status:String(fd.get("status")||"Active"),budget:Number(fd.get("budget"))||0,notes:String(fd.get("notes")||"").trim(),projectParts:collectProjectDirectParts(),manualParts:[]};
  try{
