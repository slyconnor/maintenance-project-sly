...
function purchaseOrderJobOptions(selected=""){
  ...
  if(wanted&&!sorted.some(job=>String(job.jobNo)===wanted)) html+=`<option value="${esc(wanted)}" selected>${esc(wanted)} · saved job</option>`;
  html+=sorted.map(job=>`<option value="${esc(job.jobNo||"")}"${String(job.jobNo||"")===wanted?' selected':''}>${esc(job.jobNo||"Job")} · ${esc(job.title||"Untitled")}${job.raised?` · ${esc(fmtDate(job.raised))}`:""}</option>`).join("");
  return html;
}
...
function setPurchaseOrderMeta(order){
  ...
  const values={
    poGlCode:order?.glCode||"",poDiv:order?.div||"",poDept:order?.dept||"",poAccount:selectedNewAccount,
    poDepartment:order?.department||"Maintenance",poEpp:order?.epp||"",
    poCurrency:order?.currency||appSettings.currency||"GBP",
