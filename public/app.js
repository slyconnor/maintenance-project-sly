...
async function submitPmCategory(e){e.preventDefault();const name=String(e.currentTarget.elements.name.value||"").trim();if(!name)return;try{await saveMutation("/api/preventive",{action:"category-add",name});e.currentTarget.reset();renderPmCategoryManager();renderPreventive();}catch(error){showSaveError(error);}}
