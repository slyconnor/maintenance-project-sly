...
function renderPmDialogOptions(selectedSchedule=null){
  const section=$("#pmSectionSelect"),machine=$("#pmMachineSelect"),assignees=$("#pmAssignees");if(!section||!machine||!assignees)return;
...
function openPreventiveDialog(scheduleId=""){
  ensurePreventiveUi();const dialog=$("#pmDialog"),form=$("#pmForm");if(!dialog||!form)return;form.reset();const schedule=scheduleId?preventiveSchedules.find(item=>String(item.id)===String(scheduleId)):null;renderPmDialogOptions(schedule);form.elements.id.value=schedule?.id||"";form.elements.title.value=schedule?.title||"";form.elements.description.value=schedule?.description||"";form.elements.nextDueDate.value=schedule?.nextDueDate||pmDateOnly();form.elements.intervalValue.value=Math.max(1,Number(schedule?.intervalValue)||1);form.elements.intervalUnit.value=schedule?.intervalUnit||"month";form.elements.section.value=schedule?.section||"";form.elements.machineId.value=schedule?.machineId||"";form.elements.location.value=schedule?.location||"";form.elements.active.checked=schedule?.active!==false;$("#pmDialogTitle").textContent=schedule?`Edit · ${schedule.title}`:"New PM schedule";$("#pmDialogSubtitle").textContent=schedule?"Update the recurrence, assignment or next due date.":"Create a recurring site-wide or machine-specific preventive-maintenance job.";$("#pmDeleteBtn").hidden=!schedule;$("#pmSaveBtn").textContent=schedule?"Save changes":"Save schedule";dialog.showModal();
}

async function submitPreventiveSchedule(e){e.preventDefault();const form=e.currentTarget,assigned=[...$("#pmAssignees").querySelectorAll('input[type="checkbox"]:checked')].map(input=>input.value);if(!assigned.length){alert("Assign this PM job to at least one engineer.");return;}const schedule={id:String(form.elements.id.value||""),title:String(form.elements.title.value||"").trim(),description:String(form.elements.description.value||"").trim(),nextDueDate:String(form.elements.nextDueDate.value||""),intervalValue:Number(form.elements.intervalValue.value)||1,intervalUnit:String(form.elements.intervalUnit.value||"month"),section:String(form.elements.section.value||""),machineId:String(form.elements.machineId.value||""),location:String(form.elements.location.value||"").trim(),assignedProfileIds:assigned,active:form.elements.active.checked};const btn=$("#pmSaveBtn");btn.disabled=true;btn.textContent="Saving…";try{await saveMutation("/api/preventive",{action:schedule.id?"update":"create",schedule});$("#pmDialog").close();switchView("preventive");}catch(error){showSaveError(error);}finally{btn.disabled=false;btn.textContent=schedule.id?"Save changes":"Save schedule";}}
...
function pmScheduleRow(schedule,{dueList=false}={}){const status=pmStatus(schedule);return `<tr><td><span class="pm-due-title">${esc(schedule.title)}</span>${schedule.description?`<span class="pm-due-sub">${esc(schedule.description.length>110?schedule.description.slice(0,107)+"…":schedule.description)}</span>`:""}</td><td>${esc(pmLocationLabel(schedule))}</td><td>${esc(pmAssignedNames(schedule))}</td><td>${esc(pmFrequencyLabel(schedule))}</td><td><strong>${esc(fmtDate(schedule.nextDueDate))}</strong></td><td><span class="pm-status ${status.cls}">${esc(status.label)}</span></td><td><div class="pm-table-actions">${schedule.active!==false?`<button type="button" class="btn primary compact" data-pm-complete="${esc(schedule.id)}">Complete</button>`:""}<button type="button" class="btn secondary compact" data-pm-edit="${esc(schedule.id)}">Edit</button>${dueList?"":`<button type="button" class="btn secondary compact" data-pm-toggle="${esc(schedule.id)}">${schedule.active===false?"Resume":"Pause"}</button>`}</div></td></tr>`;}

function renderPreventive(){
  if(!$("#preventiveView"))return;const visible=pmVisibleSchedules(),active=visible.filter(s=>s.active!==false),today=pmDateOnly(),week=pmWeekRange(),soonEnd=pmAddDays(today,30);const overdue=active.filter(s=>s.nextDueDate&&s.nextDueDate<today);const dueWeek=active.filter(s=>s.nextDueDate&&s.nextDueDate>=today&&s.nextDueDate<=week.end);const soon=active.filter(s=>s.nextDueDate&&s.nextDueDate>=today&&s.nextDueDate<=soonEnd);$("#pmOverdueCount").textContent=String(overdue.length);$("#pmWeekCount").textContent=String(dueWeek.length);$("#pmSoonCount").textContent=String(soon.length);$("#pmActiveCount").textContent=String(active.length);$("#pmSubtitle").textContent=selectedProfileId==="all"?"Recurring checks and planned maintenance for the whole team.":`Preventive-maintenance work assigned to ${profileContext()}.`;const dueRows=active.filter(s=>s.nextDueDate&&s.nextDueDate<=soonEnd).sort((a,b)=>String(a.nextDueDate).localeCompare(String(b.nextDueDate))||String(a.title).localeCompare(String(b.title)));$("#pmDueBody").innerHTML=dueRows.length?dueRows.map(s=>pmScheduleRow(s,{dueList:true})).join(""):`<tr><td colspan="7">No preventive-maintenance jobs are due in the next 30 days.</td></tr>`;const all=[...visible].sort((a,b)=>(a.active===false)-(b.active===false)||String(a.nextDueDate).localeCompare(String(b.nextDueDate))||String(a.title).localeCompare(String(b.title)));$("#pmScheduleBody").innerHTML=all.length?all.map(s=>pmScheduleRow(s)).join(""):`<tr><td colspan="7">No preventive-maintenance schedules yet. Press <strong>New PM schedule</strong> to create the first one.</td></tr>`;let history=[...preventiveHistory].sort((a,b)=>String(b.completedAt).localeCompare(String(a.completedAt)));if(selectedProfileId!=="all")history=history.filter(row=>(row.assignedProfileIds||[]).includes(selectedProfileId));history=history.slice(0,30);$("#pmHistoryBody").innerHTML=history.length?history.map(row=>`<tr><td>${esc(new Date(row.completedAt).toLocaleString("en-GB",{day:"2-digit",month:"short",year:"numeric",hour:"2-digit",minute:"2-digit"}))}</td><td><strong>${esc(row.title)}</strong></td><td>${esc(fmtDate(row.dueDate))}</td><td>${esc(pmHistoryLocation(row))}</td><td>${esc(row.completedByName||row.completedByEmail||"—")}</td><td>${esc(row.notes||"—")}</td></tr>`).join(""):`<tr><td colspan="6">No preventive-maintenance completions recorded yet.</td></tr>`;const emailBtn=$("#pmEmailNowBtn");if(emailBtn)emailBtn.hidden=!signedInIdentity?.admin;
}
