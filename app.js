...
function projectStats(project){
  ...
  used+=(project.manualParts||[]).reduce((sum,part)=>sum+(Math.max(0,Number(part.qty)||0)*Math.max(0,Number(part.unitPrice)||0)),0);
...
function addProjectManualPartRow(data={}){
  ...
  const supplierOptions=activeSuppliers().map(name=>`<option value="${esc(name)}" ${selectedSuppliers.has(String(name))?'selected':''}>${esc(name)}</option>`).join("");
  const alreadyInParts=Boolean(data.catalogPartId);
  row.innerHTML=`<label class="project-manual-name">Part name<input data-manual-name maxlength="180" value="${esc(data.name||"")}" placeholder="e.g. Fabricated bracket"></label><label class="project-manual-code">Part / code<input data-manual-code maxlength="100" value="${esc(data.partNo||"")}" placeholder="Optional"></label><label>Qty<input data-manual-qty type="number" min="0.01" step="0.01" value="${Math.max(0,Number(data.qty)||1)}"></label><label>Unit price (${currencySymbol()})<input data-manual-price type="number" min="0" step="0.01" value="${Math.max(0,Number(data.unitPrice)||0)}"></label><label class="project-manual-suppliers">Suppliers<select data-manual-suppliers multiple size="3">${supplierOptions}</select><span class="project-manual-help">Select one or more</span></label><label class="project-manual-stock"><input data-manual-add-parts type="checkbox" ${data.addToInventory||alreadyInParts?'checked':''}> ${alreadyInParts?'In Parts inventory':'Add to Parts inventory'}</label><button type="button" class="btn danger compact" data-project-manual-remove>Remove</button>`;
...
function renderProjectManualParts(project){
  ...
  (project?.manualParts||[]).forEach(part=>addProjectManualPartRow(part));
}
function collectProjectManualParts(){
  return $$("#projectManualParts .project-manual-part").map(row=>({id:String(row.dataset.manualId||"")||undefined,catalogPartId:String(row.dataset.catalogPartId||"")||undefined,name:String(row.querySelector("[data-manual-name]")?.value||"").trim(),partNo:String(row.querySelector("[data-manual-code]")?.value||"").trim(),qty:Math.max(0,Number(row.querySelector("[data-manual-qty]")?.value)||0),unitPrice:Math.max(0,Number(row.querySelector("[data-manual-price]")?.value)||0),suppliers:[...row.querySelectorAll("[data-manual-suppliers] option:checked")].map(option=>String(option.value||"").trim()).filter(Boolean),addToInventory:Boolean(row.querySelector("[data-manual-add-parts]")?.checked)})).filter(part=>part.name&&part.qty>0);
}
...
async function submitProject(e){
  e.preventDefault();const form=e.currentTarget,fd=new FormData(form);const project={id:String(fd.get("id")||"")||undefined,name:String(fd.get("name")||"").trim(),code:String(fd.get("code")||"").trim(),status:String(fd.get("status")||"Active"),budget:Number(fd.get("budget"))||0,notes:String(fd.get("notes")||"").trim(),manualParts:collectProjectManualParts()};
  try{
