...
function ensureProjectsUi(){
  ...
  $("#projectAddManualPartBtn")?.addEventListener("click",()=>addProjectManualPartRow());
  $("#projectManualParts")?.addEventListener("click",event=>{const remove=event.target.closest("[data-project-manual-remove]");if(remove)remove.closest(".project-manual-part")?.remove();});
  $("#projectManualParts")?.addEventListener("change",event=>{const select=event.target.closest("[data-manual-catalog]");if(!select)return;const row=select.closest(".project-manual-part");const part=partCatalog.find(item=>String(item.id)===String(select.value));if(!row)return;if(part){row.dataset.catalogPartId=String(part.id);const name=row.querySelector("[data-manual-name]"),code=row.querySelector("[data-manual-code]"),price=row.querySelector("[data-manual-price]");if(name)name.value=part.name||"";if(code)code.value=part.partNo||"";if(price&&!String(price.value||"").trim()){const remembered=lastPurchasePrice(part.name,part.partNo,"");if(remembered)price.value=Number(remembered.price).toFixed(2);}}else{row.dataset.catalogPartId="";}});
...
let projectManualPartCounter=0;
...
function addProjectManualPartRow(data={}){
  const host=$("#projectManualParts");if(!host)return;
  projectManualPartCounter+=1;
  const row=document.createElement("div");row.className="project-manual-part";row.dataset.manualId=String(data.id||"");row.dataset.catalogPartId=String(data.catalogPartId||"");
  const selectedSuppliers=new Set((Array.isArray(data.suppliers)?data.suppliers:[]).map(value=>String(value||"").trim()).filter(Boolean));
  const supplierOptionHtml=activeSuppliers().map(name=>`<option value="${esc(name)}" ${selectedSuppliers.has(String(name))?'selected':''}>${esc(name)}</option>`).join("");
  const alreadyInParts=Boolean(data.catalogPartId);
  const orderedQty=data.orderedQty!==undefined?Math.max(0,Number(data.orderedQty)||0):0;
  const usedQty=data.usedQty!==undefined?Math.max(0,Number(data.usedQty)||0):Math.max(0,Number(data.qty)||0);
  row.innerHTML=`<label class="project-manual-catalog">Existing Parts item (optional)<select data-manual-catalog>${projectCatalogOptions(data.catalogPartId||"")}</select></label><label class="project-manual-name">Part name<input data-manual-name maxlength="180" value="${esc(data.name||"")}" placeholder="e.g. Anvil"></label><label class="project-manual-code">Part code<input data-manual-code maxlength="100" value="${esc(data.partNo||"")}" placeholder="Optional"></label><label>Unit cost (${currencySymbol()})<input data-manual-price type="number" min="0" step="0.01" value="${data.unitPrice!==undefined?Math.max(0,Number(data.unitPrice)||0):""}" placeholder="0.00"></label><label>Qty ordered<input data-manual-ordered type="number" min="0" step="0.01" value="${orderedQty}"></label><label>Qty used<input data-manual-used type="number" min="0" step="0.01" value="${usedQty}"></label><label class="project-manual-suppliers">Suppliers<select data-manual-suppliers multiple size="3">${supplierOptionHtml}</select><span class="project-manual-help">Select one or more suppliers</span></label><label class="project-manual-stock"><input data-manual-add-parts type="checkbox" ${data.addToInventory||alreadyInParts?'checked':''}> ${alreadyInParts?'In Parts inventory':'Add to Parts inventory'}</label><div class="project-manual-actions"><button type="button" class="btn danger compact" data-project-manual-remove>Remove</button></div>`;
  host.appendChild(row);
}
function renderProjectManualParts(project){
  const host=$("#projectManualParts");if(!host)return;host.innerHTML="";projectManualPartCounter=0;
  const direct=Array.isArray(project?.projectParts)?project.projectParts:[];
  if(direct.length) direct.forEach(part=>addProjectManualPartRow(part));
  else (project?.manualParts||[]).forEach(part=>addProjectManualPartRow({...part,orderedQty:0,usedQty:part.qty}));
}
function collectProjectManualParts(){
  return $$("#projectManualParts .project-manual-part").map(row=>({
    id:String(row.dataset.manualId||"")||undefined,
    catalogPartId:String(row.dataset.catalogPartId||"")||undefined,
    name:String(row.querySelector("[data-manual-name]")?.value||"").trim(),
    partNo:String(row.querySelector("[data-manual-code]")?.value||"").trim(),
    orderedQty:Math.max(0,Number(row.querySelector("[data-manual-ordered]")?.value)||0),
    usedQty:Math.max(0,Number(row.querySelector("[data-manual-used]")?.value)||0),
    unitPrice:Math.max(0,Number(row.querySelector("[data-manual-price]")?.value)||0),
    suppliers:[...row.querySelectorAll("[data-manual-suppliers] option:checked")].map(option=>String(option.value||"").trim()).filter(Boolean),
    addToInventory:Boolean(row.querySelector("[data-manual-add-parts]")?.checked)
  })).filter(part=>part.name&&(part.orderedQty>0||part.usedQty>0));
}
function openProjectDialog(id=""){
  ...
  if(project){const lists=projectLinkLists(project);$("#projectPartsUsedList").innerHTML=lists.partHtml;$("#projectOrdersList").innerHTML=lists.orderHtml;$("#projectPartSearch").value="";$("#projectOrderSearch").value="";renderProjectManualParts(project);}else{renderProjectManualParts(null);}
  $("#projectDialog")?.showModal();
}
async function submitProject(e){
  e.preventDefault();const form=e.currentTarget,fd=new FormData(form);const project={id:String(fd.get("id")||"")||undefined,name:String(fd.get("name")||"").trim(),code:String(fd.get("code")||"").trim(),status:String(fd.get("status")||"Active"),budget:Number(fd.get("budget"))||0,notes:String(fd.get("notes")||"").trim(),projectParts:collectProjectManualParts(),manualParts:[]};
  try{
